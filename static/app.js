...
function bindMarketWorkspace() {
  if (!app.dataset.marketActionsBound) {
    ...
    app.addEventListener("click", async event => {
      ...
      if (control.matches("[data-market-dialog]")) { state.marketDialog = control.dataset.marketDialog; state.marketTransferRecipient = null; render(); return; }
      if (control.matches("[data-market-overview]")) { state.marketDialog = null; render(); requestAnimationFrame(() => $(".market-terminal")?.scrollIntoView({behavior:"smooth",block:"start"})); return; }
      if (control.matches("[data-close-market-dialog]")) { state.marketDialog = null; state.marketTransferRecipient = null; render(); return; }
      ...
      if (control.matches("[data-close-market]")) { state.activeApp = null; state.marketDialog = null; state.marketTransferRecipient = null; render(); await loadSession(); }
    });
  }
  ...
  $("[data-market-promo]")?.addEventListener("submit", async event => { event.preventDefault(); const result=await api("/api/wallstreet/promotions/redeem",{method:"POST",body:JSON.stringify(Object.fromEntries(new FormData(event.currentTarget)))}); toast(`${result.campaign_name}: ${result.reward_summary}`); state.marketDialog=null; await loadAppData("wallstreet"); render(); });
}
