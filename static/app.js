...
function renderMarketWorkspace() {
  ...
  return `<main class="market-workspace market-terminal-workspace market-v7">
    <header class="market-v7-top"><div class="market-v7-brand"><span>RH</span><div><strong>Ravenhood</strong><small>Faircroft private exchange</small></div></div><nav><button type="button" data-market-overview>Dashboard</button><button type="button" data-market-dialog="deposit">Deposit</button><button type="button" data-market-dialog="withdrawal">Withdraw</button><button type="button" data-market-dialog="transfer">Transfer</button><button type="button" data-market-dialog="promo">Promo</button></nav><div><em class="${data.market_open?"open":"closed"}">${data.market_open?"Market open":"Market closed"}</em><button type="button" data-refresh-market>Refresh</button><button type="button" data-close-market>Exit</button></div></header>
    ...
    <section class="market-v7-home">
      <aside class="market-v7-account"><div class="market-v7-card"><span>PORTFOLIO VALUE</span><strong>${money(total)}</strong><small>${money(account.cash_balance)} buying power</small><i><b style="width:${allocation.toFixed(1)}%"></b></i></div><div class="market-v7-actions"><button type="button" data-market-dialog="deposit">Add funds</button><button type="button" data-market-dialog="promo">Redeem code</button></div><section><header><span>WATCHLIST</span><b>${movers.length}</b></header>${movers.map(x=>`<button type="button" data-market-ticker="${escapeHtml(x.ticker)}"><span><strong>${escapeHtml(x.ticker)}</strong><small>${escapeHtml(x.name)}</small></span><em>${money(x.price)}<i class="${marketChange(x)>=0?"up":"down"}">${marketChange(x)>=0?"+":""}${marketChange(x).toFixed(2)}%</i></em></button>`).join("")}</section></aside>
      <section class="market-v7-focus"><div class="market-v7-hero"><div><span>${escapeHtml(selected.sector||"FAIRCROFT MARKET")} / ${escapeHtml(humanLabel(selected.security_type||"stock"))}</span><h1>${escapeHtml(selected.name||"Select a security")}</h1><p>${escapeHtml(selected.ticker||"—")} is trading at <b>${money(selected.price)}</b> with ${change>=0?"positive":"negative"} pressure across the Faircroft exchange.</p></div><aside><small>LAST TRADE</small><strong>${money(selected.price)}</strong><em class="${change>=0?"up":"down"}">${change>=0?"+":""}${change.toFixed(2)}%</em></aside></div>
        ...
        <div class="market-live-chart market-chart-v5 market-v7-chart ${change>=0?"positive":"negative"}"><div class="market-chart-grid"></div><svg viewBox="0 0 1000 310" preserveAspectRatio="none"><defs><linearGradient id="marketAreaV7" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="currentColor" stop-opacity=".32"/><stop offset="1" stop-color="currentColor" stop-opacity="0"/></linearGradient></defs><polygon points="0,310 ${points} 1000,310" fill="url(#marketAreaV7)"/><polyline class="market-recorded-line" points="${points}" fill="none" stroke="currentColor" stroke-width="4" vector-effect="non-scaling-stroke"/><polyline class="market-projected-line" points="${projectionPoints}" fill="none" stroke="currentColor" stroke-width="3" vector-effect="non-scaling-stroke"/><circle cx="1000" cy="${finalY}" r="7" fill="currentColor"/></svg><div class="market-chart-axis">${rangeConfig.axis.map(label=>`<span>${label}</span>`).join("")}</div><strong class="market-live-badge"><i></i>${marketRange} LIVE</strong></div>
        <section class="market-v7-discover"><header><div><span>EXPLORE</span><h2>Market movers</h2></div><strong>${securities.length} listings</strong></header><div>${featureRail.map((x,index)=>`<button type="button" class="${x.ticker===selected.ticker?"active":""}" style="--symbol-index:${index}" data-market-ticker="${escapeHtml(x.ticker)}"><b>${escapeHtml(x.ticker)}</b><strong>${escapeHtml(x.name)}</strong><small>${escapeHtml(x.sector)} · ${money(x.price)}</small><em class="${marketChange(x)>=0?"up":"down"}">${marketChange(x)>=0?"+":""}${marketChange(x).toFixed(2)}%</em></button>`).join("")}</div></section>
      </section>
...
function bindMarketWorkspace() {
  if (!app.dataset.marketActionsBound) {
    ...
    app.addEventListener("click", async event => {
      const control = event.target instanceof Element ? event.target.closest("[data-market-ticker],[data-market-range],[data-market-dialog],[data-market-overview],[data-close-market-dialog],[data-close-market],[data-refresh-market],[data-create-market-account],[data-change-market-recipient]") : null;
