...
function renderLeaderboards() {
  ...
  return `<div class="leaderboard-command leaderboard-v4 ${config.accent}">
    <header class="lb4-masthead"><div><span>${escapeHtml(config.kicker)} / LIVE DIVISION</span><h2>${escapeHtml(config.label)}</h2><p>${escapeHtml(config.unit)} · authoritative Faircroft records</p></div><aside><i></i><span><small>RANKINGS LIVE</small><strong>${rows.length} competitors</strong><em>${escapeHtml(data.generated_at ? new Date(data.generated_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "Synchronizing")}</em></span></aside></header>
    <nav class="lb4-divisions">${Object.entries(groups).map(([key, label]) => `<button type="button" class="${activeGroup === key ? "active" : ""}" data-leaderboard-group="${key}"><span>${escapeHtml(label)}</span></button>`).join("")}</nav>
    <nav class="lb4-categories">${Object.entries(LEADERBOARD_CONFIG).filter(([, item]) => item.group === activeGroup).map(([key, item], index) => `<button type="button" class="${boardKey === key ? "active" : ""}" data-leaderboard-tab="${key}"><small>${String(index + 1).padStart(2, "0")}</small><span>${escapeHtml(item.label)}</span><i></i></button>`).join("")}</nav>
    ${leader ? `<main class="lb4-field">
      <section class="lb4-leader">
        <div class="lb4-rank-mark"><span>01</span><small>CURRENT LEADER</small></div>
        <div class="lb4-leader-identity"><span>${leader.online ? "● COMPETING NOW" : "VERIFIED CONTENDER"}</span><h1>${escapeHtml(leader.name || "Unknown player")}</h1><p>${leaderboardIdentity(leader).replace(/^ · /, "")}</p></div>
        <div class="lb4-leader-score"><small>${escapeHtml(config.kicker)}</small><strong>${escapeHtml(leaderboardMetric(boardKey, leader))}</strong><span>${escapeHtml(leaderboardDetail(boardKey, leader))}</span></div>
        <div class="lb4-leader-margin"><span>LEAD ADVANTAGE</span><strong>${rows[1] ? escapeHtml(leaderboardAdvanceMetric(boardKey, leaderGap)) : "UNCONTESTED"}</strong><i><b></b></i></div>
      </section>
      <section class="lb4-chase"><header><span>THE CHASE</span><p>Closest verified competitors to the current leader</p></header>${challengers.map((item, index) => { const progress = leaderboardProgress(boardKey, rows, index + 1); return `<article><b>0${item.rank}</b><div><span>${item.online ? "LIVE" : "VERIFIED"}</span><strong>${escapeHtml(item.name || "Unknown player")}</strong><small>${leaderboardIdentity(item).replace(/^ · /, "")}</small></div><div><strong>${escapeHtml(leaderboardMetric(boardKey, item))}</strong><small>${escapeHtml(progress.label)}</small></div><i><b style="--advance:${progress.percent}%"></b></i></article>`; }).join("")}</section>
      ${personal ? `<aside class="lb4-personal"><span>YOUR LIVE POSITION</span><strong>#${personal.rank}</strong><div><b>${escapeHtml(personal.name || "Resident")}</b><small>${escapeHtml(leaderboardProgress(boardKey, rows, personalIndex).label)}</small></div><em>${escapeHtml(leaderboardMetric(boardKey, personal))}</em></aside>` : ""}
      <section class="lb4-standings"><header><div><span>FULL FIELD</span><h3>Live standings</h3></div><div><span>TRACKED <b>${Number(data.summary?.tracked_players || 0)}</b></span><span>ONLINE <b>${Number(data.summary?.online_players || 0)}</b></span><span>VERIFIED <b>${Number(data.summary?.synced_balances || 0)}</b></span></div></header><div class="lb4-table-head"><span>Rank</span><span>Competitor</span><span>Current performance</span><span>Next position</span></div>${rows.map((item, index) => { const progress = leaderboardProgress(boardKey, rows, index); return `<article class="${personal && String(item.rank) === String(personal.rank) ? "is-you" : ""}" style="--row:${index}"><strong>${String(item.rank).padStart(2, "0")}</strong><div><b>${escapeHtml(item.name || "Unknown player")}${item.online ? `<i>LIVE</i>` : ""}</b><small>${leaderboardIdentity(item).replace(/^ · /, "")}</small></div><div><b>${escapeHtml(leaderboardMetric(boardKey, item))}</b><small>${escapeHtml(leaderboardDetail(boardKey, item))}</small></div><div><span>${escapeHtml(progress.label)}</span><i><b style="--advance:${progress.percent}%"></b></i></div></article>`; }).join("")}</section>
    ...
    <footer class="lb4-source"><span>VERIFIED SOURCE</span><strong>${escapeHtml(data.sources?.[boardKey] || "Faircroft synchronized records")}</strong><p>Rankings move after every authoritative synchronization.</p></footer>
  </div>`;
...
function renderLotteryWorkspace() {
  ...
  return `<main class="lottery-workspace lottery-v4">
    ...
    <section class="lottery-v4-record">
      ...
      <aside class="lottery-v4-pool-story"><span>WHERE THE PRIZE COMES FROM</span><h2>Faircroft returns value to residents.</h2><p>The weekly pool grows from recorded public-service receipts and Ravenhood exchange contributions.</p><div><span><small>PUBLIC-SERVICE RECEIPTS</small><strong>${money(funding.fines)}</strong></span><span><small>EXCHANGE CONTRIBUTIONS</small><strong>${money(funding.market_fees)}</strong></span><span><small>AWARDS PAID</small><strong>${money(funding.awarded)}</strong></span></div>${data.latest_result?`<footer><small>LATEST OFFICIAL WINNER</small><strong>${escapeHtml(data.latest_result.winner_name||"No eligible winner")}</strong><span>${money(data.latest_result.payout_amount)} authorized</span></footer>`:`<footer><small>WINNER SPOTLIGHT</small><strong>The chamber is waiting.</strong><span>Completed drawing results will appear here.</span></footer>`}</aside>
    </section>
  </main>`;
...
function bindMarketWorkspace() {
  ...
  $$("[data-market-range]").forEach(button => button.addEventListener("click", () => { state.marketRange = button.dataset.marketRange; render(); }));
  $$("[data-market-dialog]").forEach(button => button.addEventListener("click", () => { state.marketDialog = button.dataset.marketDialog; state.marketTransferRecipient = null; render(); }));
  $("[data-close-market-dialog]")?.addEventListener("click", () => { state.marketDialog = null; state.marketTransferRecipient = null; render(); });
  $("[data-close-market]")?.addEventListener("click", async () => { state.activeApp = null; state.marketDialog = null; state.marketTransferRecipient = null; await loadSession(); });
  ...
  $("[data-change-market-recipient]")?.addEventListener("click", () => { state.marketTransferRecipient=null; render(); });
...
function renderDevLotterySettings(lottery){
  const funding=lottery.funding||{},entries=lottery.entries||[],draws=lottery.draws||[];
